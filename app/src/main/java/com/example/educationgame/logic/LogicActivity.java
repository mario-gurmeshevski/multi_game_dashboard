...
public class LogicActivity extends AppCompatActivity {
    ...
    private final int levelCount = LogicLevels.getLevelCount();
    private final int[] levelIds = new int[levelCount];
    private final int[] levelStars = new int[levelCount];
    ...
    private void seedAndLoad() {
        AppExecutors.getInstance().diskIO().execute(() -> {
            ...
            int totalPossible = levelCount * 3;
    ...
    private void seedDatabase() {
        List<GameEntity> existing = db.gameDao().getGamesByType(GameTypeEnum.LOGIC);
        int gameId;
        if (existing.isEmpty()) {
            GameEntity game = new GameEntity();
            game.setType(GameTypeEnum.LOGIC);
            game.setTitle("Logic Circuit");
            game.setDescription("Learn logic gates");
        ...
        if (levels.isEmpty()) {
            for (int i = 1; i <= levelCount; i++) {
                ...
                level.setDescription(LogicLevels.getLevel(i).getDescription());
    ...
    private void setCardClickListener(int index, boolean unlocked) {
        ...
        levelCards[index].setOnClickListener(v -> {
            Intent intent = new Intent(LogicActivity.this, LogicLevelPlayActivity.class);
            intent.putExtra(LogicLevelPlayActivity.EXTRA_LEVEL_NUMBER, levelNum);
            intent.putExtra(LogicLevelPlayActivity.EXTRA_LEVEL_ID, levelIds[index]);
